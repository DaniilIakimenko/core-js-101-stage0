...
function getCommonDirectoryPath(pathes) {
  ...
  const minParts = Math.min(...pathArrays.map((parts) => parts.length));

  for (let i = 0; i < minParts; i += 1) {
    const current = pathArrays[0][i];
    let isCommon = true;

    for (let j = 1; j < pathArrays.length; j += 1) {
      if (pathArrays[j][i] !== current) {
        isCommon = false;
        break;
      }
    }
...
function getMatrixProduct(m1, m2) {
  return m1.map((row) => {
    return m2[0].map((_, j) => {
      return row.reduce((sum, __, k) => {
        return sum + m1[m1.indexOf(row)][k] * m2[k][j];
      }, 0);
    });
  });
}
